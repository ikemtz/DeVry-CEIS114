/*
  ...
  NOV - DEC 2024
  Week 7 Assignment
  Traffic Light Controller with Emergency Mode
  Prof I. Martinez
*/
...
{ // Callback function for handling MQTT
  ...
  { // Check if the topic received is LED
    ...
    {                 // if the message received is OFF
...
{ // loop() function
  ...
  { // As long as variable iotControl is low. Run the normal traffic light
    ...
    {                                           // if crosswalk button is pressed
      ...
      while (secondsLeft > 0)
      { // As long as the timer hasn’t run up yet

        if (secondsLeft >= 10)
        { // If the timer has a value with 2 digits
